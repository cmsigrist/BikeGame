...
public final class Polygon extends Shape {
	...
	/**
	 * Creates a new polygon.
	 * @param points sequence of vertices, not null
	 */
	...
	private void initialize() {
		...
		// TODO use better convex partitioning,
		// i.e. split in convex polygons instead of only triangles
	...
	/** @return unmodifiable vertex list, not null */
    ...
    public Vector sample(Random random) {
        ...
        float offset = random.nextFloat() * area;
	...
	Part build(FixtureDef fixtureDef, Entity entity) {
		...
		//Add each fixture to the Part
		for (Vector[] vectors : components) {
			// Create Box2D polygon shape
			PolygonShape shape = new PolygonShape();
			Vec2[] vertices = new Vec2[vectors.length];
			for (int i = 0; i < vertices.length; ++i) {
				Vector v = vectors[i];
				vertices[i] = new Vec2(v.x, v.y);
			}
